...
query_that_results_in_table_of_weak_pain_killers = generate_query_that_results_in_table_of_positive_indicators_for_drug_and_undersample(
    ...
    tuple_of_concept_IDs = (1177480, 1125315, 1112807)
)

#D1
query_that_results_in_table_of_strong_pain_killers = generate_query_that_results_in_table_of_positive_indicators_for_drug_and_undersample(
...
query_that_results_in_table_of_procedures_associated_with_opioid_abuse = generate_query_that_results_in_table_of_positive_indicators_for_procedure_and_undersample(
    ...
    tuple_of_concept_IDs = (4324693, 2514433, 2514434, 2514435, 2514436, 2514437)
)

# P1
query_that_results_in_table_of_invasive_procedures = generate_query_that_results_in_table_of_positive_indicators_for_procedure_and_undersample(
...
if __name__ == "__main__":
    ...
    table_of_positive_indicators_of_symptoms_associated_with_opioid_abuse = get_data_frame(query_that_results_in_table_of_positive_indicators_of_symptoms_associated_with_opioid_abuse)
    

    print("Tables of positive indicators of drug")
    table_of_weak_pain_killers = get_data_frame(query_that_results_in_table_of_weak_pain_killers)
    table_of_strong_pain_killers = get_data_frame(query_that_results_in_table_of_strong_pain_killers)
    table_of_substance_abuse_treatments = get_data_frame(query_that_results_in_table_of_substance_abuse_treatments)

    print("Table of positive indicators of procedure")
    table_of_procedures_associated_with_opioid_abuse = get_data_frame(query_that_results_in_table_of_procedures_associated_with_opioid_abuse)
    table_of_invasive_procedures = get_data_frame(query_that_results_in_table_of_invasive_procedures)

    # -----------------------------------------------------------------------------------------------------------------------------------------------------
    # table_of_positive_indicators_of_psychiatric_condition includes anxiety, bipolar disorder, depression
    # table_of_subtance_related_issues includes non-opioid-substance-abuse, and opioid dependence
    # table_of_symptoms_associated_with_opioid_abuse include rhinitis, hypertension, and ____

    # table_of_weak_pain_killers includes aspirin, ibuprofen, acetimnophen
    # table_of_strong_pain_killers includes buprenorphine, fentanyl, morphine, oxycodone, hydromorphone, codeine, tramadol, nalbuphine, mepiridine
    # table_of_substance_abuse_treatments includes naltrexone

    # table_of_procedures_associated_with_opioid_abuse include mammography, and ED visits
    # table_of_invasive_procedures include knee, tooth, hip, vascular, head/neck, brain, and heart procedures
    
    feature_matrix = pd.merge(feature_matrix, table_of_positive_indicators_of_psychiatric_condition, how = 'left')
    feature_matrix = pd.merge(feature_matrix, table_of_positive_indicators_of_substance_related_issues, how = 'left')
    feature_matrix = pd.merge(feature_matrix, table_of_positive_indicators_of_symptoms_associated_with_opioid_abuse, how = 'left') 
    
    feature_matrix = pd.merge(feature_matrix, table_of_weak_pain_killers, how = 'left')
    feature_matrix = pd.merge(feature_matrix, table_of_strong_pain_killers, how = 'left')
    feature_matrix = pd.merge(feature_matrix, table_of_substance_abuse_treatments, how = 'left')
    
    feature_matrix = pd.merge(feature_matrix, table_of_procedures_associated_with_opioid_abuse, how = 'left')
    feature_matrix = pd.merge(feature_matrix, table_of_invasive_procedures, how = 'left')

    list_of_positive_indicator_counts_for_each_feature = [len(table_of_positive_indicators_of_psychiatric_condition), len(table_of_positive_indicators_of_substance_related_issues), len(table_of_positive_indicators_of_symptoms_associated_with_opioid_abuse), 
